	...
	var OffCanvas = $.Class.extend({
		...
		handleEvent: function(e) {
			switch (e.type) {
				...
				case 'dragend':
					if (this.isDragging) {
						var prevIsShown = this.classList.contains(CLASS_ACTIVE)
						var detail = e.detail;
						var direction = detail.direction;
...
//							}
						...
						if(this.classList.contains(CLASS_ACTIVE)!=prevIsShown){
							this._dispatchEvent()
						}
					}
					break;
	...
	$.fn.offCanvas = function(options) {
		...
		this.each(function() {
			...
			var prevIsShown = this.classList.contains(CLASS_ACTIVE)
			if (options === 'show' ){
				this.classList.contains(CLASS_ACTIVE)||offCanvasApi.show()
			}else if(options === 'close'){
				this.classList.contains(CLASS_ACTIVE)&&offCanvasApi.close()
			}else if (options === 'toggle') {
				offCanvasApi.toggle();
			}
			if(this.classList.contains(CLASS_ACTIVE)!=prevIsShown)offCanvasApi._dispatchEvent()
...
(function($, window, document, name) {
	...
	$.fn.popover = function() {
		...
		this.each(function() {
			...
			if (args[0] === 'show'){
				if(!document.querySelector('.mui-popover.mui-active')){
...
(function($, window, document, name, undefined) {
	...
	true||window.addEventListener('tap', function(e) {
